program false_position
    ...
    open(unit = 30, file = 'outA2Q3.txt', action = 'write')

    write(30, '(A14, A12, A12, A12, A16)') "Iteration NO.", "a", "b", "P_n", "|P_n - P_{n-1}|"
    write(30, '(A66)') "------------------------------------------------------------------"
    ...
    do
        ...
        if (iter == 1) then
            write(30, '(I14, 3F12.6, 8X, A)') iter, a, b, P_n, "-"
        else
            diff = abs(P_n - P_prev)
            write(30, '(I14, 3F12.6, F16.6)') iter, a, b, P_n, diff
